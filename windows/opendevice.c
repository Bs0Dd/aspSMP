...
int usbOpenDevice(usb_dev_handle **device, int vendorID, char *vendorNamePattern, int productID, char *productNamePattern, char *serialNamePattern, FILE *printMatchingDevicesFp, FILE *warningsFp, unsigned int debug)
{
struct usb_bus      *bus;
        ...
        for(dev = bus->devices; dev; dev = dev->next){  /* iterate over all devices on all busses */
            if((vendorID == 0 || dev->descriptor.idVendor == vendorID)
                ...
                if(len < 0){
                    ...
                    if (debug) printf("seen device from vendor ->%s<-\n", vendor);
                    if(shellStyleMatch(vendor, vendorNamePattern)){
                        ...
                        if(len < 0){
                            ...
                            errorCode = USBOPEN_ERR_NOTFOUND;
                            if (debug) printf("seen product ->%s<-\n", product); 
